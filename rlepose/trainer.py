...
def train(opt, cfg, train_loader, m, criterion, optimizer):
    ...
    if opt.log:
        train_loader = tqdm(train_loader, dynamic_ncols=True)#Tqdm 是一个快速，可扩展的Python进度条

    for i, (inps, labels, _, bboxes) in enumerate(train_loader):#enumerate() 函数用于将一个可遍历的数据对象(如列表、元组或字符串)组合为一个索引序列，同时列出数据和数据下标，一般用在 for 循环当中
        inps = inps.cuda()
